...
import { FiChevronDown } from "react-icons/fi"; // فلش پایین

const ProvinceCityModal = ({
  selectedProvince,
  setSelectedProvince,
  selectedCity,
  setSelectedCity
}) => {
  const [openDropdown, setOpenDropdown] = React.useState(null); // "province" | "city" | null

  return (
    <div className="p-4 max-w-md mx-auto vazir-medium">
      <Label labelName={"مبدا حرکت بیمار"}></Label>
      <div className="mb-3 flex gap-5 mt-4">
        <div className="flex-1">
          <button
            onClick={() =>
              setOpenDropdown(openDropdown === "province" ? null : "province")
            }
            className={`w-full border-gray-600 p-3 rounded-lg border text-right transition shadow-sm flex justify-between items-center
